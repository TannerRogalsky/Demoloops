...
public:
  ...
  void Update() {
    ...
    const glm::vec3 eye = glm::rotate(glm::vec3(2, 4, 20), eyeRot, glm::vec3(-0.3, 1, 0));
    ...
    for (uint32_t i = 0; i < numTris; ++i) {
      ...
      // m = glm::rotate(m, i_cycle_ratio * (float)DEMOLOOP_M_PI * 2, {0, 1, 0});
      m = glm::rotate(m, armIndex / arms * (float)DEMOLOOP_M_PI * 2, {0, 0, 1});

      float x = 0;
      x += cosf(i_cycle_ratio * DEMOLOOP_M_PI * 2 * 2) * size;
      // x *= sinf(cycle_ratio * DEMOLOOP_M_PI * 2);
      x += pow(sinf(i_cycle_ratio * DEMOLOOP_M_PI), 2) * size / 2;
      float y = 0;
      y += sinf(i_cycle_ratio * DEMOLOOP_M_PI * 2) * size * i_cycle_ratio;
      y *= cosf(cycle_ratio * DEMOLOOP_M_PI * 2);
      const float dist2D = sqrtf(x * x + y * y);
      float z = 0;
      // z -= powf(cosf(dist2D / size * DEMOLOOP_M_PI), 2) * size;
      glm::vec3 dist(x, y, z);
      m = glm::translate(m, dist);

      // colors[i] = hsl2rgbf(indexInArm, 1, 0.5);
      // colors[i] = hsl2rgbf(t / numTris, 1, 0.5);
      colors[i] = hsl2rgbf(length(dist) / size * 0.65, 1, 0.5);
