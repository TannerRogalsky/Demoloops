...
class Loop045 : public Demoloop {
public:
  Loop045() : Demoloop(CYCLE_LENGTH, 150, 150, 150), shader({shaderCode, shaderCode}) {
  ...
  void Update() {
    ...
    // const glm::vec3 eye = glm::rotate(glm::vec3(2, 4, 40), eyeRot, glm::vec3(-0.3, 1, 0));
    ...
    for (uint32_t i = 0; i < numTris; ++i) {
      ...
      m = glm::rotate(m, armIndex / arms * (float)DEMOLOOP_M_PI * arms, glm::vec3(0, 1, 0));
      m = glm::rotate(m, -i_cycle_ratio * (float)DEMOLOOP_M_PI * 0.5f, glm::vec3(0, 1, 0));
      // m = glm::rotate(m, cosf(i_cycle_ratio * (float)DEMOLOOP_M_PI * 2), glm::vec3(0, 1, 0));

      float x = sinf(i_cycle_ratio * DEMOLOOP_M_PI * 2) * 6;
      x += i_cycle_ratio * 20;
      float y = sinf(i_cycle_ratio * DEMOLOOP_M_PI * 2) * 3;
      // y *= pow(cosf(i_cycle_ratio * DEMOLOOP_M_PI), 5) * 3;
      // y *= pow(sinf(i_cycle_ratio * DEMOLOOP_M_PI * 2), 3);
      float z = sinf(i_cycle_ratio * DEMOLOOP_M_PI) * 2;
      z += 1;
      z -= pow(sinf(cycle_ratio * DEMOLOOP_M_PI), 2) * 10;
