...
class Loop043 : public Demoloop {
public:
  Loop043() : Demoloop(CYCLE_LENGTH, 150, 150, 150), shader({shaderCode, shaderCode}) {
  ...
  void Update() {
    ...
    const glm::vec3 eye = glm::rotate(glm::vec3(4, 1, 6), cycle_ratio * (float)DEMOLOOP_M_PI * 2, glm::vec3(-0.3, 1, 0));
    ...
    for (uint32_t i = 0; i < numTris; ++i) {
      ...
      m = glm::rotate(m, -i_cycle_ratio * (float)DEMOLOOP_M_PI, glm::vec3(0, 1, 0));
      // m = glm::rotate(m, i_cycle_ratio * (float)DEMOLOOP_M_PI * 2, glm::vec3(0, 1, 0));
      m = glm::translate(m, {sinf(i_cycle_ratio * DEMOLOOP_M_PI * 2) * 3, 0, cosf(i_cycle_ratio * DEMOLOOP_M_PI * 2 * 3) / 4});
