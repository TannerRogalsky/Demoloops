...
class Loop044 : public Demoloop {
public:
  Loop044() : Demoloop(CYCLE_LENGTH, 150, 150, 150), shader({shaderCode, shaderCode}) {
  ...
  void Update() {
    ...
    const glm::vec3 eye = glm::rotate(glm::vec3(2, 1, 14), (float)DEMOLOOP_M_PI / 2, glm::vec3(-0.3, 1, 0));
    ...
    for (uint32_t i = 0; i < numTris; ++i) {
      ...
      m = glm::rotate(m, cycle_ratio * (float)DEMOLOOP_M_PI * 1, glm::vec3(0, 0, 1));
      m = glm::rotate(m, armIndex / arms * (float)DEMOLOOP_M_PI * 2, glm::vec3(0, 0, 1));
      m = glm::rotate(m, -i_cycle_ratio * (float)DEMOLOOP_M_PI * 3.5f, glm::vec3(0, 1, 0));
