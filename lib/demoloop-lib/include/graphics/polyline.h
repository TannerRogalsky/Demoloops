...
protected:
  ...
  virtual void fill_color_array(Vertex *vertices);
   ...
   */
  virtual void renderEdge(std::vector<glm::vec3> &anchors, std::vector<glm::vec3> &normals,
...
public:
  ...
  void render(const float *vertices, size_t count, float halfwidth, float pixel_size, bool draw_overdraw)
  {
    ...
    memset(&this->vertices[vertex_count - 4], 0, sizeof(glm::vec3) * 4);
