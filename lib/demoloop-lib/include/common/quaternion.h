...
class Quaternion {
 ...
 public:
    ...
    Vector3 complex() const { return Vector3(mData[0], mData[1], mData[2]); }
    ...
    Vector3 euler(void) const {
      ...
      if (PI_OVER_2 - fabs(euler.y) > EPSILON) {
        euler.z = atan2(2.0 * (mData[0]*mData[1] + mData[3]*mData[2]),
                         sqx - sqy - sqz + sqw);
        euler.x = atan2(2.0 * (mData[3]*mData[0] + mData[1]*mData[2]),
                         sqw - sqx - sqy + sqz);
      } else {
  ...
  static Quaternion slerp(const Quaternion& q0, const Quaternion& q1, double t) {
    ...
    double omega = acos(q0.mData[0]*q1.mData[0] +
